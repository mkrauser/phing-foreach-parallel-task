...
* Call the task:
  The syntax is exactly the same as the original task. The only difference is a optional threadCount-Attribute, to specify the maximum number of threads / processes to use. If not specified, the library will try to guess the best number.
```xml
